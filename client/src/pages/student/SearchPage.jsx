...
const SearchPage = () => {
  ...
  const { data, isLoading, isSuccess, error } = useGetSearchCourseQuery({
    query,
  ...
  console.log({ query, categories: selectedCategories, sortByPrice });
    ...
    <div className=" max-w-7xl w-full mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-10 ">
        ...
        <div className="flex-1">
          {isLoading ? (
            [1, 2, 3].map((element, idx) => <CourseSkeleton key={idx} />)
          ) : isEmpty ? (
            <CourseNotFound />
          ) : (
